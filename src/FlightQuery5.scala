...
object FlightQuery5 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //Question 5
    def flownTogether(flightList: List[Flights1], from: LocalDate, to: LocalDate, N: Int): Seq[(Int, Int, Int, LocalDate, LocalDate)] = {
      flightList.filter(flight => flight.date.isAfter(from) && flight.date.isBefore(to)).groupBy(_.flightId)
        .flatMap { case (_, flightList) => flightList.map(_.passengerId).combinations(2).toList }
        .groupBy(identity).view.mapValues(_.size)
        .toSeq
        .filter { case (_, count) => count > N }
        .flatMap { case (comboIds, count) =>
          val List(passengerId1, passengerId2) = comboIds.sorted
          List((passengerId1, passengerId2, count, from, to))
        }
        .sortBy(_._3)(Ordering.Int.reverse)
        .toList
    }
      //Export to csv
      val output = flownTogether(flightList, LocalDate.parse("2017-01-01"), LocalDate.parse("2017-12-31"), 5 )

      val fileOut = new File("Data/Question5.csv")
      val writer = new FileWriter(fileOut)
      val csvWriter = new CSVWriter(writer)
      val header = Array("PassengerId1", "PassengerId2", "Number_Of_Flights_Together", "From", "To")
      csvWriter.writeNext(header)
      output.foreach(row => {
        val passengerId1 = row._1.toString
        val passengerId2 = row._2.toString
        val count = row._3.toString
        val from = row._4.toString
        val to = row._5.toString
        val insert = List(passengerId1, passengerId2, count, from, to).toArray
  ...
  } finally { println("CSV file created")}
